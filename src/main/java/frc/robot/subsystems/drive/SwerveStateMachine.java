...
public class SwerveStateMachine {
  public static SwerveStateMachine instance;
  ...
  public static SwerveStateMachine getInstance() {
    if (instance == null) {
  ...
  public void toggleDriveMode() {
    if (getDriveModeType() == DriveModeType.AMP) {
      setDriveMode(SwerveStateMachine.DriveModeType.SPEAKER);
    } else {
      setDriveMode(SwerveStateMachine.DriveModeType.AMP);
    }
  }
  ...
  private void calculateSpeakerHeading(Pose2d pose) {
    setHeading(new Rotation2d(
        pose.getX() - AllianceFlipUtil.apply(FieldConstants.Speaker.centerSpeakerOpening.getTranslation()).getX(),
        pose.getY() - AllianceFlipUtil.apply(FieldConstants.Speaker.centerSpeakerOpening.getTranslation()).getY()
    ));
  }
